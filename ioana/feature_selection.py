...
class FeatureSelection:
    ...
    def select_features_by_threshold(self, threshold):
        ...
        sns.heatmap(corr_matrix_train_features, annot=True, cmap='coolwarm', fmt=".2f", xticklabels=True, yticklabels=True)
        plt.title("Correlation Matrix Heatmap Train Dataset")
        plt.show()

        plt.figure(figsize=(10, 8))
        sns.heatmap(corr_matrix_test_features, annot=True, cmap='coolwarm', fmt=".2f", xticklabels=True, yticklabels=True)
        ...
        selected_features_df_train = self.train_features.drop(columns=features_to_discard_train_features)
        ...
        selected_features_df_test = self.test_features.drop(columns=features_to_discard_test_features)

        return selected_features_df_train, selected_features_df_test
    
    def filter_dataset(self, selected_features_train, selected_features_test, path, filename_train, filename_test):
...
#                            "ioana/garage/geok_squeeze_128_trans_opt/train_features.pickle", 
#                            "ioana/garage/geok_squeeze_128_trans_opt/test_features.pickle")
#     threshold = 0.75
